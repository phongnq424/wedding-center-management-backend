...
public class WeddingPackageServiceImpl implements WeddingPackageService {
    ...
    public WeddingPackageServiceImpl(
    ...
    public WeddingPackageResponse createWeddingPackage(WeddingPackageRequest request, String currentUserId) {
        ...
        DishCombo defaultCombo = loadAndValidateDefaultMenuCombo(
                request.getDefaultMenuComboId(),
                request.getMenuComboOptions()
        );
        ...
        WeddingPackage weddingPackage = WeddingPackage.builder()
                ...
                .status(request.getStatus() == null ? WeddingPackageStatus.ACTIVE : request.getStatus())
    ...
    @Override
    public WeddingPackageResponse updateWeddingPackage(
            UUID packageId,
            WeddingPackageRequest request,
        ...
        if (Boolean.TRUE.equals(weddingPackage.getIsDeleted())) {
        ...
        if (weddingPackage.getUpdatedAt() != null
                && weddingPackage.getUpdatedAt().toEpochMilli() != lastModifiedAt) {
    ...
    public List<WeddingPackageResponse> searchWeddingPackages(
            String packageName,
        ...
        if (hallTypeId != null) {
            packages = packages.stream()
                    .filter(p -> conditionRepository.findByPackageId(p.getId()).stream()
                            .anyMatch(c -> c.getHallType() != null
                                    && c.getHallType().getId().equals(hallTypeId)))
                    .collect(Collectors.toList());
        }
        ...
        if (shiftId != null) {
            packages = packages.stream()
                    .filter(p -> conditionRepository.findByPackageId(p.getId()).stream()
                            .anyMatch(c -> c.getShiftId() != null
                                    && c.getShiftId().equals(shiftId)))
                    .collect(Collectors.toList());
        }
    ...
    public List<WeddingPackageResponse> getAllWeddingPackages() {
        return weddingPackageRepository.findAllActive()
                .stream()
                .map(this::mapToResponse)
                .collect(Collectors.toList());
    }
    ...
    private void validateServiceItems(List<WeddingPackageServiceItemDTO> serviceItems) {
        ...
        for (WeddingPackageServiceItemDTO item : serviceItems) {
            ...
            com.wedding.management.domain.service.model.Service service =
                    serviceRepository.findByIdAndIsDeletedFalse(item.getServiceId())
    ...
    private void validateBeverageAllowances(List<WeddingPackageBeverageAllowanceDTO> allowances) {
        ...
        for (WeddingPackageBeverageAllowanceDTO item : allowances) {
            if (item.getBeverageId() == null) {
                throw new BadRequestException("MSG2: Đồ uống không được để trống");
            }

            if (!beverageIds.add(item.getBeverageId())) {
                throw new BadRequestException("MSG49: Danh sách đồ uống không được trùng lặp");
            }

            if (item.getAllowanceQuantity() == null || item.getAllowanceQuantity() <= 0) {
                throw new BadRequestException("MSG13: Số lượng đồ uống phải lớn hơn 0");
            }

            Beverage beverage = beverageRepository.findByIdAndIsDeletedFalse(item.getBeverageId())
                    .orElseThrow(() -> new ResourceNotFoundException("Đồ uống không tồn tại"));

            if (beverage.getStatus() != BeverageStatus.ACTIVE) {
                throw new BadRequestException("MSG2: Đồ uống không còn hoạt động");
            }
        }
    }
    ...
    private void validateBenefits(List<WeddingPackageBenefitDTO> benefits) {
        ...
        for (WeddingPackageBenefitDTO benefit : benefits) {
            if (benefit.getItemType() == null) {
                throw new BadRequestException("MSG2: Loại quà tặng không được để trống");
            }

            if (benefit.getItemId() == null) {
                throw new BadRequestException("MSG2: Item quà tặng không được để trống");
            }

            if (benefit.getQuantity() == null || benefit.getQuantity() <= 0) {
                throw new BadRequestException("MSG13: Số lượng quà tặng phải lớn hơn 0");
            }

            String uniqueKey = benefit.getItemType() + ":" + benefit.getItemId();

            if (!uniqueKeys.add(uniqueKey)) {
                throw new BadRequestException("MSG49: Danh sách quà tặng không được trùng lặp");
            }

            validateBenefitItemExistsAndActive(benefit.getItemType(), benefit.getItemId());
        }
    }

    private void validateBenefitItemExistsAndActive(PackageBenefitItemType itemType, UUID itemId) {
        switch (itemType) {
            case DISH -> {
                Dish dish = dishRepository.findByIdAndIsDeletedFalse(itemId)
                        .orElseThrow(() -> new ResourceNotFoundException("Món ăn quà tặng không tồn tại"));

                if (dish.getStatus() != DishStatus.ACTIVE) {
                    throw new BadRequestException("MSG2: Món ăn quà tặng không còn hoạt động");
                }
            }

            case SERVICE -> {
                com.wedding.management.domain.service.model.Service service =
                        serviceRepository.findByIdAndIsDeletedFalse(itemId)
                                .orElseThrow(() -> new ResourceNotFoundException("Dịch vụ quà tặng không tồn tại"));

                if (service.getStatus() != ServiceStatus.ACTIVE) {
                    throw new BadRequestException("MSG2: Dịch vụ quà tặng không còn hoạt động");
                }
            }

            case BEVERAGE -> {
                Beverage beverage = beverageRepository.findByIdAndIsDeletedFalse(itemId)
                        .orElseThrow(() -> new ResourceNotFoundException("Đồ uống quà tặng không tồn tại"));

                if (beverage.getStatus() != BeverageStatus.ACTIVE) {
                    throw new BadRequestException("MSG2: Đồ uống quà tặng không còn hoạt động");
                }
            }
        }
    }
    ...
    private void validateConditions(List<WeddingPackageConditionDTO> conditions) {
        ...
        for (WeddingPackageConditionDTO condition : conditions) {
            if (condition.getConditionType() == null) {
                throw new BadRequestException("MSG2: Loại điều kiện gói không được để trống");
            }

            if (condition.getConditionType() == PackageConditionType.HALL_TYPE
                    && condition.getHallTypeId() == null) {
                throw new BadRequestException("MSG2: Loại sảnh trong điều kiện không được để trống");
            }

            if (condition.getConditionType() == PackageConditionType.SHIFT
                    && condition.getShiftId() == null) {
                throw new BadRequestException("MSG2: Ca tiệc trong điều kiện không được để trống");
            }

            if ((condition.getConditionType() == PackageConditionType.MIN_TABLES
                    || condition.getConditionType() == PackageConditionType.MAX_TABLES)
                    && (condition.getNumericValue() == null || condition.getNumericValue() <= 0)) {
                throw new BadRequestException("MSG13: Giá trị số bàn trong điều kiện phải lớn hơn 0");
            }
        }
    }
    ...
    private void savePackageMenuCombos(WeddingPackage weddingPackage, List<UUID> comboIds, String currentUserId) {
        for (UUID comboId : comboIds) {
            ...
            WeddingPackageMenuCombo entity = WeddingPackageMenuCombo.builder()
    ...
    private void savePackageBeverageAllowances(
            WeddingPackage weddingPackage,
            List<WeddingPackageBeverageAllowanceDTO> allowances,
            String currentUserId
    ) {
        for (WeddingPackageBeverageAllowanceDTO item : allowances) {
            Beverage beverage = beverageRepository.findByIdAndIsDeletedFalse(item.getBeverageId())
            ...
            WeddingPackageBeverageAllowance entity = WeddingPackageBeverageAllowance.builder()
                    .weddingPackage(weddingPackage)
                    .beverage(beverage)
                    .allowanceQuantity(item.getAllowanceQuantity())
                    .note(item.getNote())
                    .createdBy(currentUserId)
                    .createdAt(Instant.now())
                    .isDeleted(false)
    ...
    private void savePackageBenefits(
            WeddingPackage weddingPackage,
            List<WeddingPackageBenefitDTO> benefits,
            String currentUserId
    ) {
        if (benefits == null) return;
        ...
        for (WeddingPackageBenefitDTO benefit : benefits) {
            BenefitItemSnapshot snapshot = resolveBenefitItemSnapshot(
                    benefit.getItemType(),
                    benefit.getItemId()
            );

            int quantity = benefit.getQuantity() == null ? 1 : benefit.getQuantity();
            double unitValue = snapshot.unitValue();
            double totalValue = unitValue * quantity;

            WeddingPackageBenefit entity = WeddingPackageBenefit.builder()
                    .weddingPackage(weddingPackage)
                    .itemType(benefit.getItemType())
                    .itemId(snapshot.itemId())
                    .itemName(snapshot.itemName())
                    .quantity(quantity)
                    .unitValue(unitValue)
                    .totalValue(totalValue)
                    .customerPayAmount(0.0)
                    .note(benefit.getNote())
                    .displayOrder(benefit.getDisplayOrder() == null ? order : benefit.getDisplayOrder())
                    .createdBy(currentUserId)
                    .createdAt(Instant.now())
                    .isDeleted(false)
                    .build();
    ...
    private record BenefitItemSnapshot(
            UUID itemId,
            String itemName,
            Double unitValue
    ) {}

    private BenefitItemSnapshot resolveBenefitItemSnapshot(
            PackageBenefitItemType itemType,
            UUID itemId
    ) {
        return switch (itemType) {
            case DISH -> {
                ...
                yield new BenefitItemSnapshot(
                        dish.getId(),
                        dish.getName(),
                        dish.getUnitPrice() == null ? 0.0 : dish.getUnitPrice()
                );
            }
            ...
            case SERVICE -> {
                ...
                yield new BenefitItemSnapshot(
                        service.getId(),
                        service.getName(),
                        service.getPrice() == null ? 0.0 : service.getPrice()
                );
            }
            ...
            case BEVERAGE -> {
                ...
                yield new BenefitItemSnapshot(
                        beverage.getId(),
                        beverage.getName(),
                        beverage.getUnitPrice() == null ? 0.0 : beverage.getUnitPrice()
                );
            }
        };
    }

    private void savePackageConditions(
            WeddingPackage weddingPackage,
            List<WeddingPackageConditionDTO> conditions,
            String currentUserId
    ) {
        if (conditions == null) return;
        ...
        for (WeddingPackageConditionDTO condition : conditions) {
            ...
            if (condition.getHallTypeId() != null) {
                hallType = hallTypeRepository.findById(condition.getHallTypeId())
                        .orElseThrow(() -> new ResourceNotFoundException("Loại sảnh không tồn tại"));

                if (Boolean.TRUE.equals(hallType.getIsDeleted()) || hallType.getStatus() != HallTypeStatus.ACTIVE) {
                    throw new BadRequestException("MSG2: Loại sảnh trong điều kiện không còn hoạt động");
                }
            }

            WeddingPackageCondition entity = WeddingPackageCondition.builder()
                    .weddingPackage(weddingPackage)
                    .conditionType(condition.getConditionType())
                    .hallType(hallType)
                    .shiftId(condition.getShiftId())
                    .shiftName(condition.getShiftName())
                    .numericValue(condition.getNumericValue())
                    .conditionValue(condition.getConditionValue())
                    .displayOrder(condition.getDisplayOrder() == null ? order : condition.getDisplayOrder())
                    .createdBy(currentUserId)
                    .createdAt(Instant.now())
                    .isDeleted(false)
                    .build();
    ...
    private boolean packageContainsAllDishCombos(UUID packageId, List<UUID> comboIds) {
        Set<UUID> existingIds = menuComboRepository.findByPackageId(packageId)
                .stream()
                .map(m -> m.getDishCombo().getId())
    ...
    private boolean packageContainsAllServices(UUID packageId, List<UUID> serviceIds) {
        Set<UUID> existingIds = serviceItemRepository.findByPackageId(packageId)
                .stream()
                .map(WeddingPackageServiceItem::getServiceId)
                .collect(Collectors.toSet());
    ...
    private boolean packageContainsAllBeverages(UUID packageId, List<UUID> beverageIds) {
        Set<UUID> existingIds = beverageAllowanceRepository.findByPackageId(packageId)
                .stream()
                .map(b -> b.getBeverage().getId())
                .collect(Collectors.toSet());
    ...
    private WeddingPackageResponse mapToResponse(WeddingPackage weddingPackage) {
        ...
        List<UUID> comboIds = menuCombos.stream()
                .map(m -> m.getDishCombo().getId())
                .collect(Collectors.toList());

        List<String> comboNames = menuCombos.stream()
                .map(m -> m.getDishCombo().getName())
                .collect(Collectors.toList());
        ...
        double menuDiscountSavingsAmount = Math.max(0.0, originalMenuComboPrice - discountedMenuComboPrice);

        double benefitGiftValue = benefits.stream()
                .mapToDouble(b -> b.getTotalValue() == null ? 0.0 : b.getTotalValue())
                .sum();

        double originalPackageTotal =
                originalMenuComboPrice
                        + includedServiceTotal
                        + beverageAllowanceTotal
                        + benefitGiftValue;

        double estimatedPackageTotal =
                discountedMenuComboPrice
                        + includedServiceTotal
                        + beverageAllowanceTotal;
        ...
        List<WeddingPackageServiceItemDTO> serviceDTOs = services.stream()
                .map(s -> WeddingPackageServiceItemDTO.builder()
                        .id(s.getId())
        ...
        List<WeddingPackageBeverageAllowanceDTO> beverageDTOs = beverages.stream()
                .map(b -> WeddingPackageBeverageAllowanceDTO.builder()
                        .id(b.getId())
                        .beverageId(b.getBeverage().getId())
                        .beverageName(b.getBeverage().getName())
                        .allowanceQuantity(b.getAllowanceQuantity())
                        .note(b.getNote())
                        .build())
                .collect(Collectors.toList());

        List<WeddingPackageBenefitDTO> benefitDTOs = benefits.stream()
                .map(b -> WeddingPackageBenefitDTO.builder()
                        .id(b.getId())
                        .itemType(b.getItemType())
                        .itemId(b.getItemId())
                        .itemName(b.getItemName())
                        .quantity(b.getQuantity())
                        .unitValue(b.getUnitValue())
                        .totalValue(b.getTotalValue())
                        .customerPayAmount(b.getCustomerPayAmount())
                        .note(b.getNote())
                        .displayOrder(b.getDisplayOrder())
                        .build())
                .collect(Collectors.toList());

        List<WeddingPackageConditionDTO> conditionDTOs = conditions.stream()
                .map(c -> WeddingPackageConditionDTO.builder()
                        .id(c.getId())
                        .conditionType(c.getConditionType())
                        .hallTypeId(c.getHallType() == null ? null : c.getHallType().getId())
                        .hallTypeName(c.getHallType() == null ? null : c.getHallType().getName())
        ...
        String menuSummary = String.join("\n", comboNames);

        String serviceSummary = serviceDTOs.stream()
                .map(s -> (s.getServiceName() == null ? String.valueOf(s.getServiceId()) : s.getServiceName())
                        + " x" + (s.getQuantity() == null ? 1 : s.getQuantity()))
                .collect(Collectors.joining("\n"));

        String beverageSummary = beverageDTOs.stream()
                .map(b -> b.getBeverageName() + " x" + b.getAllowanceQuantity())
                .collect(Collectors.joining("\n"));

        String benefitSummary = benefitDTOs.stream()
                .map(b -> b.getItemName()
                        + " x" + (b.getQuantity() == null ? 1 : b.getQuantity())
                        + " - khách trả 0đ")
                .collect(Collectors.joining("\n"));

        String conditionSummary = conditionDTOs.stream()
                .map(c -> c.getConditionType() + ": "
                        + (c.getHallTypeName() != null
                        ? c.getHallTypeName()
                        : c.getShiftName() != null
                        ? c.getShiftName()
                        : c.getNumericValue() != null
                        ? c.getNumericValue().toString()
                        : c.getConditionValue()))
                .collect(Collectors.joining("\n"));
        ...
        return WeddingPackageResponse.builder()
                ...
                .numberOfBenefits(benefits.size())
                ...
                .menuDiscountSavingsAmount(menuDiscountSavingsAmount)
    ...
    private double calculateOriginalComboPrice(DishCombo combo) {
        ...
        return combo.getSlots()
                .stream()
                .filter(slot -> slot.getDefaultDish() != null)
                .mapToDouble(slot -> slot.getDefaultDish().getUnitPrice() == null
                        ? 0.0
                        : slot.getDefaultDish().getUnitPrice())
                .sum();
    }
    ...
    private double calculateDiscountedComboPrice(DishCombo combo) {
        ...
        return originalPrice * (1 - combo.getComboDiscountRate() / 100.0);
    }
    ...
    private double calculateBeverageAllowanceTotal(List<WeddingPackageBeverageAllowance> beverages) {
        ...
        return beverages.stream()
                .mapToDouble(item -> {
                    ...
                    double price = item.getBeverage().getUnitPrice() == null
                            ? 0.0
                            : item.getBeverage().getUnitPrice();

                    int quantity = item.getAllowanceQuantity() == null
                            ? 0
                            : item.getAllowanceQuantity();
