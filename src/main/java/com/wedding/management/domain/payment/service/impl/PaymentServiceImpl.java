...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentServiceImpl(
            ...
            InvoicePaymentSyncService invoicePaymentSyncService,
    ...
    public PaymentResponse cancelPayment(
        ...
        if (!StringUtils.hasText(reason)) {
            throw bad("MSG2: Lý do hủy không được để trống");
        }

        Payment payment = getPayment(paymentId);

        if (payment.getStatus() != PaymentStatus.UNPROCESSED) {
            throw bad("MSG67: Chỉ payment UNPROCESSED mới được hủy");
        }
        ...
        saveAuditLog(
                ...
                "CANCEL_PAYMENT",
                saved.getId(),
                saved.getBooking().getCustomerName()
        );
    ...
    @Override
    public PaymentOtpChallengeResponse createProcessPaymentOtp(
            UUID paymentId,
            String currentUserId
    ) {
        Payment payment = getPayment(paymentId);
        ...
        return twoFactorVerificationService.createPaymentProcessingChallenge(
                paymentId,
                currentUserId
        );
    }
    ...
    public PaymentResponse processPayment(
        ...
        double confirmedBefore = safe(
                paymentRepository.sumAmountByBookingAndStatus(
                        booking.getId(),
        ...
        if (payment.getPaymentType() == PaymentType.DEPOSIT
                && paymentRepository.countByBookingAndTypeAndStatus(
                booking.getId(),
                PaymentType.DEPOSIT,
                PaymentStatus.PROCESSED
        ) > 0) {
        ...
        if (booking.getStatus() == BookingStatus.PENDING
                && safe(booking.getConfirmedPaymentAmount()) + EPS >= safe(booking.getDepositAmount())) {
            booking.setStatus(BookingStatus.CONFIRMED);
        }

        bookingRepository.save(booking);

        invoicePaymentSyncService.syncPaymentStatusForBooking(
                booking.getId(),
                currentUserId
        );
    ...
    private void recalculateBookingPaymentSummary(Booking booking) {
        ...
        booking.setRemainingAmount(
                Math.max(0, safe(booking.getBookingAmount()) - confirmed)
        );
    ...
    private void checkVersion(Payment p, long client) {
        if (p.getUpdatedAt() != null
                && p.getUpdatedAt().toEpochMilli() != client) {
            throw new ResponseStatusException(
                    HttpStatus.CONFLICT,
                    "MSG62: Dữ liệu đã được người khác cập nhật"
            );
        }
    }
    ...
    private PaymentSummaryResponse summary(Booking b) {
        return PaymentSummaryResponse.builder()
                ...
                .bookingAmount(safe(b.getBookingAmount()))
                .depositAmount(safe(b.getDepositAmount()))
                .confirmedPaidAmount(safe(b.getConfirmedPaymentAmount()))
                .pendingPaymentAmount(safe(
                        paymentRepository.sumAmountByBookingAndStatus(
                                b.getId(),
                                PaymentStatus.UNPROCESSED
                        )
                ))
                .remainingAmount(safe(b.getRemainingAmount()))
                .build();
    }
    ...
    private PaymentResponse toResponse(Payment p) {
        ...
        return PaymentResponse.builder()
                ...
                .lastModifiedAt(p.getUpdatedAt())
                .lastModifiedBy(String.valueOf(p.getUpdatedBy()))
                .build();
    }
    ...
    private boolean match(PaymentSearchCriteria c, Payment p) {
        ...
        return contains(p.getId(), c.getPaymentId())
                && contains(b.getId(), c.getBookingId())
                && contains(b.getCustomerName(), c.getCustomerName())
                && contains(b.getCustomerPhone(), c.getCustomerPhone())
                && (c.getPaymentType() == null || p.getPaymentType() == c.getPaymentType())
                && (c.getPaymentMethod() == null || p.getPaymentMethod() == c.getPaymentMethod())
                && (c.getStatus() == null || p.getStatus() == c.getStatus())
                && (c.getAmountFrom() == null || p.getAmount() >= c.getAmountFrom())
                && (c.getAmountTo() == null || p.getAmount() <= c.getAmountTo())
                && contains(p.getReferenceNumber(), c.getReferenceNumber())
                && (c.getPaymentDateFrom() == null
                || (p.getPaymentDate() != null
                && !p.getPaymentDate().isBefore(c.getPaymentDateFrom())))
                && (c.getPaymentDateTo() == null
                || (p.getPaymentDate() != null
                && !p.getPaymentDate().isAfter(c.getPaymentDateTo())));
    }

    private boolean contains(UUID id, String q) {
        return !StringUtils.hasText(q)
                || (id != null
                && id.toString().toLowerCase().contains(q.toLowerCase()));
    }
